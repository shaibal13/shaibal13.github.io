...
!function(n){
    ...
    n("header a").click(function(e){
        // Check if the link is an internal link (i.e., it starts with a #)
        var target = n(this).attr("href");
        if (!n(this).hasClass("no-scroll") && target.startsWith("#")) { 
            e.preventDefault();
            var i = n(target).offset().top;
            n("html, body").animate({scrollTop:i+"px"}, +Math.abs(window.pageYOffset - n(target).offset().top));
